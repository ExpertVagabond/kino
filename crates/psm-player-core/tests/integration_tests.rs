//! Integration tests for PSM Player Core

use psm_player_core::{
    PlayerConfig, PlayerState, Resolution,
    PsmColors, PsmTheme, CssVariables,
...
fn test_css_variables() {
    ...
    assert!(css.contains("--psm-primary: #9b30ff"));
...
fn test_srt_to_vtt_conversion() {
    let srt = "1\n00:00:00,000 --> 00:00:04,000\nHello!";
    let vtt = psm_player_core::captions::srt_to_vtt(srt);
...
"#;
    ...
    let active = psm_player_core::captions::cues_at_time(&cues, 2.5);
    assert_eq!(active.len(), 1);
    assert_eq!(active[0].text, "First");

    // At 7.5 seconds, second cue should be active
    let active = psm_player_core::captions::cues_at_time(&cues, 7.5);
    assert_eq!(active.len(), 1);
    assert_eq!(active[0].text, "Second");

    // At 12 seconds, no cue should be active
    let active = psm_player_core::captions::cues_at_time(&cues, 12.0);
...
fn test_chapter_contains_time() {
    let chapter = psm_player_core::Chapter::new("ch1", "Introduction", 0.0, 60.0);
...
fn test_chapter_duration() {
    let chapter = psm_player_core::Chapter::new("ch1", "Introduction", 10.0, 70.0);
...
fn test_media_tracks_chapter_at() {
    use psm_player_core::{MediaTracks, Chapter};
