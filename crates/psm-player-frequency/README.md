# PSM Player Frequency Analysis

High-performance audio frequency analysis library for Purple Squirrel Media's decentralized video platform.
...
```toml
[dependencies]
psm-player-frequency = { path = "../psm-player-frequency", features = ["full"] }
```
...
```rust
use psm_player_frequency::{AudioAnalyzer, FrequencyAnalyzer};
...
```rust
use psm_player_frequency::fingerprint::Fingerprinter;
...
```rust
use psm_player_frequency::tagging::ContentTagger;
...
```rust
use psm_player_frequency::streaming::{StreamAnalyzer, StreamConfig};
...
```rust
use psm_player_frequency::recommend::RecommendationEngine;
...
The PSM Player CLI includes frequency analysis commands:

```bash
# Analyze frequencies
psm-player frequency input.wav --top-k 10 --json

# Generate fingerprint
psm-player fingerprint input.wav --output fingerprint.json

# Auto-tag content
psm-player autotag input.wav --max-tags 5

# Generate thumbnail
psm-player thumbnail video.mp4 --output thumb.jpg

# Find similar content
psm-player similar input.wav --library /path/to/media --limit 10

# Full processing pipeline
psm-player process video.mp4 --output results.json
```
...
```javascript
import init, { PsmFrequencyAnalyzer, PsmFingerprinter } from 'psm-player-wasm';

await init();

const analyzer = new PsmFrequencyAnalyzer(44100, 4096);
...
```bash
cargo bench --package psm-player-frequency
```
